...
export default class DataProxy {
  constructor(name, settings) {
    ...
    // if (settings.reportType === 1 && settings.row && settings.row.len === 1) {
    //   this.settings.showToolbar = false
    // }
    // save data begin
      ...
      if (property === 'merge') {
        ...
        selector.range.each((ri, ci) => {
          ...
          if (property === 'format') {
            ...
            let cell = rows.getCellOrNew(selector.ri, selector.ci);
            cell[property] = value
          }
        });
      }
  ...
  merge() {
    ...
    selector.ci = selector.range.sci
    selector.ri = selector.range.sri
  }
